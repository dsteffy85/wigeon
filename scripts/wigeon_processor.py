...
class WigeonProcessor:
    ...
    def process_file(self, file_path, third_party_name, third_party_email, 
                    report_type=None, report_date=None, email_subject=None, 
        ...
        try:
            ...
            if not third_party:
                print(f"   Creating new third-party record...")
                third_party_id = self.db.add_third_party(
                    name=third_party_name,
                    email=third_party_email,
                    description=f"Auto-created from report ingestion"
                )
            else:
            ...
            if normalized_rows:
                ...
                print(f"   ⚠️  No data rows found in file")
    ...
    def query_data(self, report_id=None, third_party_name=None, 
                   limit=100, offset=0):
        """Query report data"""
    ...
    def export_data(self, output_path, report_id=None, third_party_name=None, 
                   format='csv'):
        """
        Export data to file
        ...
        if format == 'json':
            ...
            fieldnames = ['report_id', 'third_party_name', 'report_type', 
                         'report_date', 'row_number'] + sorted(all_keys)
            ...
            try:
                ...
                headers = ['report_id', 'third_party_name', 'report_type', 
                          'report_date', 'row_number'] + sorted(all_keys)
                ws.append(headers)
