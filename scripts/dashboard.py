    ...
    def print_recent_reports(self, limit=10):
        ...
        if results:
            ...
            for row in results:
                ...
                type_short = (report_type or "Unknown")[:28] + ".." if report_type and len(report_type) > 30 else (report_type or "Unknown")
                
                print(f"{report_id:4d} {name_short:20s} {type_short:30s} {report_date or 'N/A':12s} {row_count:8,} {status:10s}")
            print()
...
Examples:
  ...
  python3 dashboard.py --days 14
    ...
    parser.add_argument('--db', default='database/wigeon.db',
                       help='Path to database file (default: database/wigeon.db)')
    ...
    parser.add_argument('--search', metavar='TERM',
                       help='Search reports by filename or type')
    parser.add_argument('--detail', type=int, metavar='ID',
                       help='Show details for report ID')
